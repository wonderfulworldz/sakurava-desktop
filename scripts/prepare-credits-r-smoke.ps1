...
if (Test-Path -LiteralPath $databasePath) {
    throw "Disposable fixture already exists at $databasePath. Inspect it or choose a new -Root; this script will not overwrite it."
}

Push-Location $repositoryRoot
try {
    cargo run --manifest-path .\src-tauri\Cargo.toml --bin credits_r_smoke -- prepare --root $runtimeRoot
