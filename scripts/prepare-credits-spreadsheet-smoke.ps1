...
$Root = if ([string]::IsNullOrWhiteSpace($Root)) {
    Join-Path $repositoryRoot 'manual-smoke\runtime-data\credits-spreadsheet-07'
} else {
    $Root
}
...
if (Test-Path -LiteralPath $databasePath) {
    throw "Disposable Credits spreadsheet fixture already exists at $databasePath. Inspect it or choose a new -Root; this script will not overwrite it."
}

Push-Location $repositoryRoot
try {
    cargo run --manifest-path .\src-tauri\Cargo.toml --bin credits_r_smoke -- prepare-spreadsheet --root $runtimeRoot
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    node .\scripts\prepare-credits-spreadsheet-artifacts.cjs --root $runtimeRoot
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
finally {
    Pop-Location
