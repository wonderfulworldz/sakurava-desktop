...
## Current Adaptive Routing Reconciliation — 2026-08-29

For every substantial executable Codex prompt, visibly select the lowest-cost
combination likely to complete the work correctly in one controlled cycle:
...
Use proportional governance: Quick Path for low-risk deterministic work,
Standard Path for ordinary multi-file/integration work, and Guarded Path for
high-risk data, security, package, destructive, or unexplained cross-system
work. Result Review is required before dependent execution. Backlog or stage
...
Use explicit progress and quota posture only for work that is meaningfully
multi-stage, long-running, measurement-heavy, retry-heavy, experimental, or
quota-sensitive. Ordinary short tasks do not require progress percentages.
When progress is useful, define stable outcome/task/gate denominators; commands,
retries, prompts, files, and test counts are not progress. Report the next
highest-value action and explain any denominator change.
...
- `Low` for deterministic inspection or documentation;
...
Do not use Extra High only because the batch is important.
...
|---|---:|---:|---|
| Git or worktree inspection | A | Low | AUDIT ONLY |
| Documentation update | A | Low | IMPLEMENT |
...
`Why` must be one concise sentence based on:
...
Progress percentages and quota posture accompany model metadata only when the
work meets the proportional progress criteria above. Model selection remains
subordinate to approval, scope, Active Locks, and expected-value execution
control. Capability tier may be recorded internally when useful but is not a
required operator-facing field.
...
Mode: IMPLEMENT
Risk: MEDIUM
Why: The task is a controlled multi-file implementation with strong focused tests and no migration or stored-data change.
```
