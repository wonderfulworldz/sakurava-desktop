...
## LOCK-VIDEO-001 — Adopted mpv Composition Architecture

**Status:** ACTIVE

The technically adopted built-in Video Player uses mpv 0.41 through shared
libmpv, D3D11 Composition, DirectComposition, and WebView2
CompositionController in a dedicated `sakurava-media-host.exe`. The main
process supplies trusted source identity through typed/versioned IPC. The
engine owns authoritative playback truth; React owns presentation and transient
UI state. Technical adoption does not grant legal/public-distribution approval.

---

## LOCK-VIDEO-002 — One Authoritative Main/PiP Session

**Status:** ACTIVE

Main and the real borderless, Always-on-Top PiP window use one media-host, one
libmpv context/session, and one source load. PiP transfers presentation rather
than creating another decoder. Return restores Main; PiP close does not
terminate playback; no Notification bell belongs in the media window.

---

## LOCK-VIDEO-003 — Engine Must Fit the Accepted Overlay UI

**Status:** ACTIVE

Engine selection is composition-first. The `MEASURED` libVLC 3.x child-HWND
route was rejected for this architecture because it covered the React/WebView
overlay and intercepted pointer input; this is not a claim that VLC is
generally defective. Do not rescue that route by moving accepted controls
outside video, creating a second transparent overlay OS window, continuously
copying decoded frames through CPU memory, replacing global Tauri/WRY, adding a
second PiP decoder, or redesigning the accepted UI around an engine. GStreamer
work is not required after final mpv adoption unless a separately approved
reassessment establishes new evidence.
...
Sakurava has one AppShell-level upper-right Notification bell and compact
Notification Center, with no dedicated Notification page or route. Active
progress is producer-owned; determinate percentages require authoritative
current/total values and otherwise remain indeterminate. Terminal history is
local operational data retained for 30 days with a maximum of 500 terminal
items and excluded from `.skv`. Persisted unconfirmed running state becomes
`interrupted`, not automatically failed. Additional producers remain
separately gated.
...
Use a top-level-safe existence posture only. Do not enumerate, recurse, count,
search, open, inspect, move, delete, stage, commit, clean, or otherwise expose
`manual-smoke/` children unless a future explicit authorization defines the
exact evidence boundary. Never alter protected evidence merely to match an
older recorded count.
...
Canonical authority precedence is `00`, `02`, `03`, `01`, `07`, `04`, `06`,
`05`, then the boot prompt. The restored `00`–`07` Project OS is canonical.
The V2 `PROJECT.md`, `STATE.md`, `DECISIONS.md`, `LESSONS.md`, `HISTORY.md`, and
`BACKLOG.md` files remain secondary transitional evidence pending separately
approved disposition; they do not override canonical owners. A generated
`sakurava-desktop-CHATGPT-BRAIN.md` is a downstream Project ChatGPT source
snapshot, not repository authority. The established external exporter is
`C:\Users\Working WW\Documents\AI-Workflow\Export-ChatGPTBrain.ps1`, invoked
with the repository through `-ProjectRoot`; its external output and the
operator-controlled Project ChatGPT source replacement do not acquire
