...
recovery. The detailed restored roadmap below remains historical continuity;
former `Current`, `Next`, active-stage, progress, and permission fields below
do not override this reconciliation unless repeated here.

Current posture:

- Active Application Batch: `NONE`;
...
- Project Brain V2 migration at
  `c82a02f17732ccea941ee072b5842907f32b6057` was implemented and treated as
  completed at that time;
- the later recovery audit found the old canonical responsibilities only
  partially represented by V2;
- explicit operator approval restored and reconciled canonical `00`–`07`, the
  ledger archive, and the boot prompt;
- V2 documents remain secondary transitional evidence pending separate
  disposition.

Completed governance/tooling milestones:

- canonical Project OS recovery and reconciliation — `COMPLETED_AND_ACCEPTED`;
- external Project ChatGPT exporter recovery — `COMPLETED_AND_ACCEPTED`;
- canonical nine-input exporter adaptation — `COMPLETED_AND_ACCEPTED`;
- canonical Brain generation and validation — `COMPLETED_AND_ACCEPTED`;
- operator-controlled Project ChatGPT Source refresh —
  `COMPLETED_OBSERVED_BY_OPERATOR` (`OBSERVED_BY_OPERATOR`).

Accepted post-migration product/technical milestones retained:

- Simple Statistics — `COMPLETED_AND_ACCEPTED_WITH_RUNTIME_LIMITATION`;
- dependency/security hygiene —
  `COMPLETED_AND_ACCEPTED_WITH_RESIDUAL_FINDINGS`;
- Automatic Mini Images —
  `COMPLETED_AND_ACCEPTED_WITH_RUNTIME_LIMITATION`;
- Remove Mini Images — `COMPLETED_AND_ACCEPTED_WITH_RUNTIME_LIMITATION`;
...
1. architecture audit and mpv D3D11 Composition proof — `PASS`;
2. Stage 1 — `COMPLETED_AND_ACCEPTED`;
3. Stage 2 — `COMPLETED_AND_ACCEPTED`;
4. Stage 3 — `PARTIAL_ACCEPTED`;
5. Stage 3 Closure — `COMPLETED_AND_ACCEPTED`, delivery candidate validated;
6. final technical mpv adoption — `APPROVED`.

There is no Stage 4. The adopted architecture is mpv 0.41/shared libmpv,
D3D11 Composition, DirectComposition, WebView2 CompositionController, and one
dedicated Sakurava media-host with one authoritative Main/PiP
context/session/source. The libVLC 3.x child-HWND route is rejected only for
the accepted overlay composition architecture; GStreamer or a libVLC retry is
not required without a future separately approved reassessment.

Recorded production-candidate identity:

- mpv `v0.41.0`, source commit
  `41f6a645068483470267271e1d09966ca3b9f413`;
- FFmpeg `n9.0.1`, source commit
  `bf1b838f2ab88b4f8fd83443325c782ea0e0f7fa`;
- candidate libmpv SHA-256
  `6F312FD78D309B389436307C29066F227046FC64CEC5061D027DCE802BF91286`;
- profile: `gpl=false`, shared libmpv, cplayer disabled, D3D11/D3D11 hardware
  acceleration enabled, OpenGL/Vulkan/Lua/JavaScript disabled;
- classification: `TECHNICALLY_ADOPTED_ENGINE` and
  `LEGAL/LICENSE_REVIEW_REQUIRED_BEFORE_DISTRIBUTION`.

`gpl=false` is not legal approval or public-redistribution clearance.
...
`PARTIAL_BASELINE_ACCEPTED`. R1 is completed and closed with
`BUILD_STRATEGY_VALIDATED_WITH_REBUILD_REQUIRED`; R2 is
...
`7e5fc6e7b807047203e645256b2f20f87a298f81`
...
The implementation and accepted disposable smoke are complete as reported.
No live AppData or existing-catalog repair occurred.

Closure outcome: ordinary Video, Image, and Performer deletion is reference-
safe; dependent Credits and inbound relationships are cleaned transactionally;
validation occurs before commit; failures roll back; public aliases and
high-water counters remain protected; Bulk CSV/XLSX parent Delete infers
required cleanup; and standard Form and Import/Export workflows remain
unchanged.

Deferred future work remains outside this batch: selected empty Export sections
should later export as valid empty sheets/sections, while Credits spreadsheet
UX, terminology, and Managed Categories wording require dedicated product and
compatibility decisions. The current spreadsheet contract remains unchanged.
...
- database preparation scaling from about 1.0 second at Dataset S to about
  35.5 seconds at Dataset A;
...
Batch `42.2A — Lossless Translation Storage Foundation` is complete as reported and remains unused by current runtime callers. Exact snapshots, journal, rollback, pending inspection, explicit recovery, and recovery-export primitives are available; focused verification and the production build passed. No current Translation behavior, migration, or caller integration occurred. The next proposed sub-stage is `42.2B — English-Only Language Identity and Resolution`, separately gated. Runtime Translation smoke remains separately gated by disposable WebView-profile isolation.
...
- Batch `41.9` remains `COMPLETED_AND_CLOSED`;
- completed Batch `41.9` work must not be repeated;
- unresolved containment questions belong to Batch `42.2`;
- release-facing Translation completion belongs to Batch `42.11`.

### Completed Batch 42.1

Batch `42.1 — GitHub and Repository Health Triage` is `COMPLETED_AND_CLOSED`.

Closure commit:

`528246899386f960a1cce0b6f4bc4cba03b5315b`

The Codex closure report states that:

- exactly nine approved Project OS authority files were committed;
- local and remote `main` synchronized at the closure commit;
- tracked worktree and staging were clean;
- `manual-smoke/` remained local and untracked;
- no source, dependency, workflow, package, test, build, or remediation change was included.

The closure report is recorded as:

`REPORTED_SYNCHRONIZED_BY_CODEX_CLOSURE`

Batch `42.1` found no immediate production security blocker.

Its classified findings remain assigned to:

- Batch `42.12` for later repository and governance decisions;
...
### Completed Batch 42.3A — Catalog Reference Integrity and Deletion Recovery

Batch `42.3A` is completed and closed. Its final implementation baseline is
`7e5fc6e7b807047203e645256b2f20f87a298f81`.
...
- 42.3A-2 status: `COMPLETED_REPORTED`;
- 42.3A-3 status: `PASSED_OBSERVED_BY_OPERATOR`;
- implementation, tests, builds, runtime, migration, dependency, and live-data
  repair permission: `false`;
- Batch `42.3`: `READY_PENDING_SEPARATE_APPROVAL` and audit-first.

The former deletion-integrity blocker is resolved for future prevention.
Spreadsheet deletion contracts remain protected. Restore and Backup scope
remain assigned to Batches `42.6` and `42.7`.
...
- repeat completed Batch `41.9` corrective work without evidence;
- absorb Batch `42.11` release completion;
...
- applicable Active Locks;
- architecture and data evidence sources;
- persistence, fallback, CSV, compatibility, and migration questions;
- data-safety rules;
- historical separation for Batches `41.9`, `42.2`, and `42.11`;
- stop conditions;
- final report format;
- Definition of Done.
...
- Translation data and live AppData access remain prohibited;
- tests and builds remain prohibited;
- Vite and Tauri development execution remain prohibited;
- dependency remediation and migration remain prohibited;
- repository mutation requires an immediate stop.

The authoritative active scope is recorded in:
